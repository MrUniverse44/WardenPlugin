...
public final class CustomConfiguration {
    ...
    private final CustomConfiguration defaults;
    ...
    public CustomConfiguration(CustomConfiguration defaults) {
        this(new LinkedHashMap<String, Object>(), defaults);
    }

    CustomConfiguration(Map<?, ?> map, CustomConfiguration defaults) {
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            ...
            if (entry.getValue() instanceof Map) {
                this.self.put(key, new CustomConfiguration(
                        (Map<?,?>) entry.getValue(),
    ...
    private CustomConfiguration getSectionFor(String path) {
        ...
        if (section == null) {
            section = new CustomConfiguration((defaults == null) ? null : defaults.getSection(root));
            self.put(root, section);
        }

        return (CustomConfiguration) section;
    }
    ...
    public <T> T get(String path, T def) {
        CustomConfiguration section = getSectionFor(path);
        Object val;
        if (section == this)
        {
            val = self.get(path);
        ...
        if (val == null && def instanceof CustomConfiguration)
        {
            self.put(path, def);
        }
    ...
    public void set(String path, Object value) {
        if (value instanceof Map) {
            value = new CustomConfiguration((Map<?,?>) value, (defaults == null) ? null : defaults.getSection(path));
        }
    ...
    public CustomConfiguration getSection(String path) {
        Object def = getDefault(path);
        return (CustomConfiguration) get(path, (def instanceof CustomConfiguration) ? def : new CustomConfiguration((defaults == null) ? null : defaults.getSection(path)));
    }
